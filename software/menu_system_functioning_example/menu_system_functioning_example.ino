...
void demoMenu() {
  ...
  oledMenu.menuTitle = "SERVO DMX v0.1";     // menus title (used to identify it)
  ...
  oledMenu.menuItems[8] = "Display Off";
}  // demoMenu
...
void menuActions() {
  ...
  if (oledMenu.menuTitle == "SERVO DMX v0.1") {    // actions when an item is selected in "demo_menu"

    // demonstrate quickly create a menu from a list
    if (oledMenu.menuTitle == "SERVO DMX v0.1" && oledMenu.selectedMenuItem == 3) {
...
void setup() {
  ...
  // display greeting message - pressing button will start menu
    displayMessage("DELED", "DMX Servo V0.1\nController");
    display.display();
}
